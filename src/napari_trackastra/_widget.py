...
def _track_function(model, imgs, masks, mode="greedy", **kwargs):
    ...
    track_graph, masks_tracked = model.track(
        ...
        # if not get_settings().application.ipy_interactive
        # else tqdm,
        **kwargs,
    )  # or mode="ilp"
...
class Tracker(Container):
    def __init__(
        ...
        self._save_button = PushButton(
            label="Save results\n(masks-tracked + tracks layers)",
            visible=False,
        )
        self._save_path = FileEdit(
            label="Save path",
            mode="d",
            value=str(Path.home() / "TRA"),
            visible=False,
        )
        self._save_format = ComboBox(
            label="Output format",
            choices=("CTC", "GEFF"),
            value="CTC",
            visible=False,
        ...
        self._save_format.changed.connect(self._update_path_suggestion)
    ...
    def _run(self, event=None):
        ...
        self._save_format.show()
        self._save_button.show()

    def _save(self, event=None):
        pm = npe2.PluginManager.instance()

        outdir = Path(self._save_path.value)
        if self._save_format.value == "GEFF":
            root = zarr.open_group(outdir, mode="w")
            masks = self._viewer.layers["masks_tracked"].data
            ...
            writer_contrib = pm.get_writer(
                str(outdir / "tracking_graph"),
                ["tracks"],
                "napari-geff",
            )[0]
            save_layers(
                path=str(outdir / "tracking_graph"),
                layers=[self._viewer.layers["tracks"]],
                plugin="napari-geff",
                _writer=writer_contrib,
            )

        elif self._save_format.value == "CTC":
            writer_contrib = pm.get_writer(
                str(outdir),
                ["labels", "tracks"],
                "napari-ctc-io",
            ...
            raise ValueError(f"Unknown format {self._save_format.value}")

    def _update_path_suggestion(self, event=None):
        if self._save_format.value == "CTC":
            suggestion = Path.home() / "TRA"
        elif self._save_format.value == "GEFF":
            suggestion = Path.home() / "tracked.zarr"
        else:
            raise ValueError(f"Unknown format {self._save_format.value}")

        self._save_path.value = suggestion
